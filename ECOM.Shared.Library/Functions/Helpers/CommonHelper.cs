...
namespace ECOM.Shared.Library.Functions.Helpers
{
	...
	{
        /// <summary>
        /// Executes an asynchronous action with retry logic. If the action fails, it will be retried
        /// up to the specified maximum number of attempts, with a delay between retries.
        /// </summary>
        /// <param name="action">The asynchronous action to execute.</param>
        /// <param name="retryInterval">The time interval to wait between retries.</param>
        /// <param name="maxAttemptCount">The maximum number of times to attempt the action (default is 3).</param>
        public static async Task RetryAsync(Func<Task> action, TimeSpan retryInterval, int maxAttemptCount = 3)
        {
            ...
            for (int attempted = 0; attempted < maxAttemptCount; attempted++)
            {
                try
                {
                    ...
                    await action();
                    return; // Action completed successfully, exit the retry loop
                }
                catch (Exception ex)
                {
                    exceptions.Add(ex);
            ...
            throw new AggregateException($"Action failed after {maxAttemptCount} attempts.", exceptions);
        }
